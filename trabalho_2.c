...
int busca_veiculo(char placa[], estacionamento E){
	...
	for(i =0; i < NUM_PILHAS; i++){ 
		...
		veiculo *p = E.P[i].topo->prox; 

		/*Procure enquanto não chegar no ultimo elemetno*/
		while(p->prox != NULL){
			if(strcmp(placa, p->placa) == 0){
				return i; // caso tenha achado retorne o indice da pilha..
			}
			p = p->prox;
		}
		/*Procurando no ultimo elemento.*/
		if(strcmp(placa, p->placa) == 0){
			return i; 
		}		
